...
class SaleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Jika terdapat parameter pencarian, filter data penjualan berdasarkan nama pelanggan
        if ($search) {
            $sales = Sale::whereHas('customer', function ($query) use ($search) {
                $query->where('name', 'like', "%$search%");
            })->get();
        } else {
            // Jika tidak ada parameter pencarian, ambil semua data penjualan
            $sales = Sale::all();
        }
    
        return view('pages.sale.index', compact('sales', 'search'));
    }
